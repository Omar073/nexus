...
This file is the **canonical** Flutter/Dart AI guidance for the Nexus repo. Cursor loads it via **`.cursor/rules/nexus.mdc`**, which points here—avoid maintaining parallel copies of the same rules under `.cursor/rules/`.
...
* **Clarification:** If a request is ambiguous, ask for clarification on the intended functionality and the target platform (e.g., command-line, web, server, desktop, or mobile).
* **Correction:** I don't know everything so please correct me when I'm wrong.
...
* **Comments:** Write clear comments for complex or non-obvious code. Avoid over-commenting. Do not add too much comments, just the needed ones that explain the why not the how. Same for readme files.
* **Trailing Comments:** Don't add trailing comments.
* **Async/Await:** Ensure proper use of async/await for asynchronous operations with robust error handling. Use Futures, async, and await for asynchronous operations. Use Streams for sequences of asynchronous events.
...
* **Localization:** Always make sure that every text is properly localized (no hard-coded text).
* **Dynamic Content:** Make sure content is dynamic, no hard-coded values/lists, placeholders. This is to be a production app.
...
* **Logical Layers:** Organize the project into logical layers: Presentation (widgets, screens), Domain (business logic classes), Data (model classes, API clients), Core (shared classes, utilities, and extension types).
...
* **Built-in Solutions:** Prefer Flutter's built-in state management solutions. Do not use a third-party package unless explicitly requested. Bloc/Cubit can be considered if the app requires it as the main methodology.
...
* **Build Runner:** If the project uses code generation, ensure that build_runner is listed as a dev dependency in pubspec.yaml. Use build_runner for all code generation tasks, such as for json_serializable.
...
* **Requirement:** Please add unit tests whenever possible.
...
* **Requirement:** Responsiveness and sleek UI/UX is very important.
...
* **Network Images:** Use NetworkImage or Image.network for images loaded from the network. Always include loadingBuilder and errorBuilder for a better user experience.
...
* **Cached Images:** For cached images, use a package like cached_network_image with NetworkImage.
* **Custom Icons:** Use ImageIcon to display an icon from an ImageProvider, useful for custom icons not in the Icons class.
* **Error Handling for Network Images:** When using network images, always provide an errorBuilder.
...
* **Text Fields:** Configure textCapitalization, keyboardType, and other properties appropriately.

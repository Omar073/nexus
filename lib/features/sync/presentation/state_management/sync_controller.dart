...
/// UI state for sync: queue count, last sync time, and conflict lists.
/// Conflict streams use task/note data types (from sync handlers); this feature depends on them for resolution UI.
class SyncController extends ChangeNotifier {
  SyncController({required SyncService syncService})
    ...
    _confSub =
        (_syncService.getConflictStream('task')
                as Stream<List<SyncConflict<Task>>>)
            .listen(replaceConflicts);
    _noteConfSub =
        (_syncService.getConflictStream('note')
                as Stream<List<SyncConflict<Note>>>)
            .listen(replaceNoteConflicts);

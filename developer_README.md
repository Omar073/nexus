...
- **Notification dedup: `notifiedAt` field**
  ...
  - Reminder **Snooze** / **Complete** use `showsUserInterface: false` so Android uses a broadcast + headless Dart isolate (`onBackgroundNotificationResponse`) and actions can run **without launching the app**. On some devices the OS may still misroute taps (e.g. open the app with a body-style intent and no `actionId`); that is an OEM/plugin limitation, not something we can fix without `showsUserInterface: true` (which would open the app).
